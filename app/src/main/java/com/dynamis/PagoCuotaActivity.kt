...
class PagoCuotaActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tituloNavbar.text = "Pago de Cuota"
    ...
    private fun mostrarDialogoDeAyuda() {
        ...
        builder.setTitle("Ayuda: Pago de Cuota")
        builder.setMessage(
            "Esta pantalla es solo para registrar el pago de cuotas de Socios.\n\n" +
                    "1. Ingrese el DNI del socio.\n" +
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                if (cursor != null && cursor.moveToFirst()) {
                    ...
                    val isSocio = cursor.getInt(cursor.getColumnIndexOrThrow(UsuarioEntry.COLUMN_IS_SOCIO)) == 1
                    ...
                    if (isSocio) {
                        ...
                        withContext(Dispatchers.Main) {
                            if (exitoPago) {
                                Toast.makeText(this@PagoCuotaActivity, "Pago de cuota registrado", Toast.LENGTH_SHORT).show()
                                limpiarCampos()
                            } else {
                                Toast.makeText(this@PagoCuotaActivity, "Error al guardar el pago", Toast.LENGTH_LONG).show()
                            }
                        }
                    } else {
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@PagoCuotaActivity, "Error: El DNI $dniStr no pertenece a un socio.", Toast.LENGTH_LONG).show()
                        }
                    }

                } else {
                    withContext(Dispatchers.Main) {
                        Toast.makeText(this@PagoCuotaActivity, "Error: Usuario con DNI $dniStr no encontrado", Toast.LENGTH_LONG).show()
                    }
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@PagoCuotaActivity, "Error: ${e.message}", Toast.LENGTH_LONG).show()
                }
            } finally {
                cursor?.close()
            }
